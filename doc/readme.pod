...
L<http://101companies.org/wiki/Contribution:dancer> for details.
...
C<cpan Dancer Data::Structure::Util Data::UUID File::Slurp JSON::XS Moops> (or replace
C<cpan> with whatever the command for your installer is called).
...
To run the intersting feature of this contribution, namely the web UI, once again C<cd>
into the project's folder, run C<dancer101.plx> and then point your browser to
L<http://localhost:3000/>, preferably with JavaScript enabled.
